...
pub fn day4_part2() {
    let s = fs::read_to_string("./src/day4/input.txt").unwrap();
    let mut lines: Vec<String> = s.lines().map(|i| i.to_string()).collect();
        ...
        for i in 0..lines.len() {
            ...
            for j in 0..len_line{
                ...
                if cur_lin[j] == '@' {
                    ...
                    if count < 4 {
                       a_roll_exists = true; 
                       let mut new_line: Vec<char> = lines[i].chars().collect();
                       new_line[j] = '.';
                       lines[i] = new_line.iter().collect();
                       total += 1;
                    }
                }
            }
